<?php include_once('header.php')?>
<!-- Body Content Wrapper -->
<div class="ms-content-wrapper">

	<div class="row">
		<div class="col-md-12">
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb pl-0">
					<li class="breadcrumb-item"><a href="index.php">Home</a></li>
                    <li class="breadcrumb-item"><a href="Data_&_Reports_sub.php">Data & Reports</a></li>
                    <li class="breadcrumb-item"><a href="studentfeedefaulter.php">Student Wise Fee Defaulters</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Saleem</li>
                </ol>
            </nav>
        </div>
        <div class="col-md-8 m-auto">
            <div class="ms-panel w-100">
              <div class="ms-panel-header">
                <div class="row">
                    <div class="col-md-4">
                       <div class="dropdown float-left">
                        <h5>Name : <span>Mr XYZ</span></h5>
                        <h5>Batch : <span>PG-Pink</span></h5>
                    </div>
                </div>
                <div class="col-md-6">
                   <div class="dropdown float-left">
                    <h5>Admission no : <span>2323</span></h5>
                    <h5>Total Amount : <span>1212</span></h5>
                </div>
            </div>
            <div class="col-md-2">
               <div class="dropdown float-right">
                <button class="btn btn-primary" onclick="window.location.href=''">Export as CSV</button>
            </div>
        </div>
    </div>
</div>
</div>
</div>
<div class="col-md-12 text-center content-center">
    <div class="ms-panel w-100">
        <div class="ms-panel-body">
            <div class="">
                <table class="table table-hover thead-primary">
                    <thead>
                        <tr>
                            <th scope="col">No.</th>
                            <th scope="col" class="text-left">Name</th>
                            <th scope="col" class="text-left">Start Date</th>
                            <th scope="col" class="text-left">Due Date</th>
                            <th scope="col" class="text-left">Students</th>
                            <th scope="col" class="text-left">Balance</th>
                            
                        </tr>
                    </thead>
                    <tbody>
                        <tr>

                            <td>
                             1
                         </td>
                         <td class="text-left"><a href="studentfeesdefaulterdetail.php">For the Year 2020</a></td>
                         <td class="text-left">29/02/2020</td>
                         <td class="text-left">29/02/2021</td>
                         <td class="text-left">22</td>
                         <td class="text-left">323232323</td>
                     </tr>
                 </tbody>
             </table>
         </div>
     </div>
 </div>
</div>

</div>
</div>

<!-- footer -->

<?php include_once('footer.php')?>



